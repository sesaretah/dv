...
var base_url = 'http://194.225.14.164/users/password/edit?reset_password_token='
...
async.series([
    function(callback) {
      var q = "SELECT * FROM users WHERE id = " + argv['id']
        connection.query(q, function (error, results, fields) {
          console.log(results[0].reset_password_token);
          receiver = results[0].email
          url = base_url + results[0].reset_password_token
          callback(null, 'one');
        });
