      ...
      async.waterfall([
        function(callback) {
          ...
          $('a').each(function(index){
            ...
            if (obj.children[0] !== undefined && obj.children[0].children[0].data !== undefined ){
              ...
              var q = "SELECT * FROM categories WHERE title = '" + obj.children[0].children[0].data + "'"
              connection.query(q, function (error, results, fields) {
                //  console.log(results[0]['id']);
                if (error) throw error;
                if (results.length == 0){
                  connection.query('INSERT INTO categories SET ?', { title: obj.children[0].children[0].data, stage: index}, function (error, results, fields) {
                    if (error) throw error;
                  });
                }
              });
        ...
        function(arg1, callback) {
          ...
          $('.c-params__list-key').each(function(index){
            var obj = this
            if (obj.children[0] !== undefined && obj.children[0].children[0].data !== undefined ){
              var q = "SELECT * FROM specs WHERE title = '" + obj.children[0].children[0].data +"'"
              connection.query(q, function (error, results, fields) {
                if (error) throw error;
                if (results.length == 0){
                  ...
                  connection.query(q, function (error, results, fields) {
                    if (error) throw error;
                    connection.query('INSERT INTO specs SET ?', { title: obj.children[0].children[0].data, category_id: results[0]['id']}, function (error, results, fields) {
                      if (error) throw error;
                    });
                  });
                }
