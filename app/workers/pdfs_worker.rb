    ...
    if !article.blank?
      ...
      for upload in Upload.where("uploadable_type = ? AND uploadable_id =  ? AND attachment_type IN  (?) AND printable is true", "Article", id, ["article_citation", "article_attachment", "article_documents"])
        if upload.attachment_content_type == "application/pdf"
          system("convert -density 600 #{Rails.root}/public/pdfs/#{id}/#{uuid}.pdf #{upload.attachment.path} -resize 50% #{Rails.root}/public/pdfs/#{id}/#{uuid}.pdf")
        end
      end
      for kinship in article.kinships
        ...
        #p "#{kin_uploads}"
        if kin_uploads != " "
          system("convert -density 600 #{Rails.root}/public/pdfs/#{id}/#{uuid}.pdf #{kin_uploads} -resize 50% #{Rails.root}/public/pdfs/#{id}/#{uuid}.pdf")
        end
      end
