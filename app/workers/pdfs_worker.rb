  ...
  def perform(id, uuid, type = "raw_print")
    ...
    if !article.blank?
      ...
      for kinship in article.kinships
        ...
        #p "#{kin_uploads}"
        if kin_uploads != " "
          system("convert -density 200 #{Rails.root}/public/pdfs/#{id}/#{uuid}.pdf #{kin_uploads}  -background white -alpha remove -compress zip #{Rails.root}/public/pdfs/#{id}/#{uuid}.pdf")
        end
      end
