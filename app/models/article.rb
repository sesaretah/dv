class Article < ActiveRecord::Base
  ...
  def self.in_dashboard(user, home_setting)
    ...
    if home_setting.archived.blank? || home_setting.archived == 0
      archived = '( archived is NULL or archived = 1 or archived = 0 )'
    end
    ...
    if ['coming ASC', 'coming DESC'].include?(order)
      ...
      articles = where("workflow_state_id in (?)  and #{archived}", workflow_state_ids).order(order)
    end
    articles
  end
  ...
  def textual
    text = "#{title} <br />"
    titlings.group_by(&:title_type_id).each do |_k, _v|
      text += "#{t.content} <br />"
    end
    text += '<br /><br />'
