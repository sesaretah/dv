class NotificationMailer < ApplicationMailer
    ...
    def notify_email(user_id, notify_type, notifier, notify_text, custom_text, auxiliary_custom_text=nil)
        ...
        case notify_type
        when  'article_sent'
            @body = "#{t(:an_article)} #{t(:with_title)} #{notify_text}  #{t(:via)} #{notifier} #{t(:article_sent_notification)} "
            #@second_pargraph =  "#{custom_text}"
            @title = "#{t(:new_article_sent)} #{truncate(notify_text)}"
        when  'article_received'
            @body = "#{t(:an_article)} #{t(:with_title)} #{notify_text} #{t(:via)} #{notifier} #{t(:article_received_notification)}  "
            #@second_pargraph =  "#{custom_text}"
